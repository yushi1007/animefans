...
import { fetchStreamingUrls, fetchAnimeDetails } from '../../data/fetchAnimeApi'
...
const AnimeVideo = () => {
    ...
    const animeId = id.replace(id.substring(id.indexOf('-episode') + 1),"").slice(0, -1)

    useEffect(() => {
        fetchStreamingUrls(`vidcdn/watch/${id}`)
        .then((data)=>setVideo(data.sources));

        fetchAnimeDetails(animeId).then(data=>data).then((data)=>{
          console.log(data)
